...
local.starRedMail = function() {
	...
	DBRemote.timerFlag2 = setInterval(local.sendRedMail,24 * 60 * 60 * 1000)
}
//代理分红邮件发放
			...
			DBRemote.dbService.getPlayerObject(agencyId,"agencyRedList",function(data) {
				if(data){
					...
					for(var index in data.list){
						...
						mailInfo.content += "ID" +  index + "玩家消耗分红 " +  data.list[index] + " 金币\t\n"
					}
...
DBRemote.prototype.addAgncyDivide = function(agencyDivides,cb) {
	...
	if(!DBRemote.timerFlag){
    	...
    	// console.log(tmpDate)
    	// console.log("=================================================222")
    	setTimeout(local.starRedMail,tmpDate)
	}
